...
declare global {
    ...
    class Hooks {
        static on(event: string, callback: (message: ChatMessage, options: object, userId: string) => void): number;
    ...
    class Config {
        ...
        TextEditor: {
            enrichers: {
                pattern: RegExp,
                enricher: (match: RegExpMatchArray, options: {}) => Promise<HTMLElement | null>,
            }[]
        }
        Dice: {
            rolls: Roll[];
    ...
    class Token extends FoundryObject {
        uuid: string;
        actor?: Actor;
        scene: Scene;
        texture: {
    ...
    class User {
        ...
        uuid: string;
        name: string;
        targets: Array<Token> & Set<Token> & { first(): Token };
        isGM: boolean;
        isActiveGM: boolean;
    }
